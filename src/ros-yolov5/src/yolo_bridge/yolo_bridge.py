...
import sys
sys.path.append('/home/robot/ros_ws/src/ros-yolov5/model')
...
class Ros2Yolo:
    def __init__(self, node_name='demo_server', name='yolo'):
       ...
       # self.model = rospy.get_param('yolov5/model', 'yolov5m')
        #self.weight = rospy.get_param('yolov5/weight', None)
        self.model = '/home/robot/ros_ws/src/ros-yolov5/model'
        self.weight = '/home/robot/ros_ws/src/ros-yolov5/model/yolov5m.pt'
        self.device = rospy.get_param('yolov5/device', 'cpu')
    ...
    def load_model(self):
        ...
        if self.yolo5_path is None:
            self.model = torch.hub.load('ultralytics/yolov5', 'custom', path='/home/robot/ros_ws/src/ros-yolov5/model/yolov5m.pt')
            #self.model = self.model.cuda() if self.device != 'cpu' else self.model
            #self.model = attempt_load(self.weight, self.device, yolo_path='/home/robot/ros_ws/src/ros-yolov5/')
        else:
            self.model = attempt_load(self.weight, self.device, yolo_path='/home/robot/ros_ws/src/ros-yolov5')
        ...
        for i, det in enumerate(pred):
            ...
            if len(det):
                ...
                for c in det[:, -1].unique():
                    ...
                    s += f"{self.names[int(c)]}{'s' * (n > 1)}: {n}"  # add to string
